...
Data yang digunakan dalam kesempatan kali ini adalah data Air Quality di New Delhi periode 21-24 Oktober 2024 pada setiap jam. Dengan x1 yaitu CO2 dan x2 yaitu NO2

```{r}
library(readxl)
data <- read_excel("C:/KULYAHH/SEMESTER 5/MPDW/TUGAS/PRAK 2/data2.xls", sheet = "dt1")
View(data)
```

## Eksplorasi Data

Sebelum melakukan regresi, akan diperlihatkan *plot time-series* dari CO2 dan NO2 
...
ts.plot(data.ts1, xlab="Time Period ", ylab="CO2", main= "Time Series Plot of CO2")
points(data.ts1)
...
#model regresi
model <- lm(AQI~CO+o3, data=data)
summary(model)
```

Model yang dihasilkan adalah $$y_i=-52.82068+0.37264x1_t+105.68304x2_t$$ Berdasarkan ringkasan model dapat diketahui bahwa hasil uji F memiliki *p-value* \< $\alpha$ (5%). Artinya, minimal terdapat satu variabel yang berpengaruh nyata terhadap model. Hasil uji-t parsial kedua parameter regresi, yaitu intersep dan koefisien regresi juga menunjukkan hal yang sama, yaitu memiliki *p-value* \< $\alpha$ (5%) sehingga nyata dalam taraf 5%. Selanjutnya dapat dilihat juga nilai $R^2=0.7186$. Artinya, sebesar 71.86% keragaman nilai AQI dapat dijelaskan oleh peubah CO dan NO2. Hasil ini menunjukkan hasil yang cukup bagus, seolah mendapatkan hasil baik. Namun, kita perlu melakukan uji terhadap sisaannya seperti berikut ini.
...
no2.trans<- data$o3[-1]-data$o3[-72]*rho

modelCOmanual<- lm(aqi.trans~co.trans+no2.trans)
